 ...
 *
 * This file is part of NHA Help Desk.
 *
 * NHA Help Desk is free software; you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * NHA Help Desk is distributed in the hope that it will be useful,
...
/**
 * Wizard to configure and initialize the NHA Help Desk application
 */
...
$oP = new SetupPage('NHA Help Desk email test utility');
...
function CheckEmailSetting($oP)
{
	...
	{
		case 'PHPMail':
		$oP->info("NHA Help Desk is configured to use PHP's <a style=\"background:transparent;padding:0;color:#000;text-decoration:underline;\" href=\"http://www.php.net/manual/en/function.mail.php\" target=\"_blank\">mail</a> function to send emails.");
		...
		case 'SMTP':
		$oP->info("NHA Help Desk is configured to use the <b>SMTP</b> transport.");
		...
		$oP->info("SMTP configuration (from config-NHA Help Desk.php): host: $sHost, port: $sPort, user: $sDisplayUserName, password: $sDisplayPassword, encryption: $sDisplayEncryption.");
		if (($sHost == 'localhost') && ($sPort == '25') && ($sUserName == '') && ($sPassword == '') )
		{
			$oP->warning("The default settings may not be suitable for your environment. You may want to adjust these values by editing NHA Help Desk's configuration file (".utils::GetConfigFilePathRelative().").");
		}
		break;
		
		case 'Null':
		$oP->warning("NHA Help Desk is configured to use the <b>Null</b> transport: emails sending will have no effect.");
		$bRet = false;
		break;
		
		case 'LogFile':
		$oP->warning("NHA Help Desk is configured to use the <b>LogFile</b> transport: emails will <em>not</em> be sent but logged to the file: 'log/mail.log'.");
		$bRet = true;
		break;
	...
	if ($bConfigAsync)
	{
		$oP->warning("NHA Help Desk is configured to send emails <em>asynchronously</em>. Make sure that cron.php is scheduled to run in the background, otherwise regular emails will <em>not</em> be sent. For the purpose of this test, the email will be sent <em>synchronously</em>.");
	}
...
function DisplayStep1(SetupPage $oP)
{
	...
	$oP->add("<h1>NHA Help Desk email test</h1>\n");
...
function DisplayStep2(SetupPage $oP, $sFrom, $sTo)
{
	...
	$oP->add("<h1>NHA Help Desk configuration wizard</h1>\n");
	...
	$oEmail->SetSubject("Test NHA Help Desk");
	$oEmail->SetBody("<p>Hello,</p><p>The email function is now working fine.</p><p>You may now be able to use the notification function.</p><p>NHA Help Desk</p>");
